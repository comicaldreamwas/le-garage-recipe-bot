...
// True when two equal-length strings differ only by ONE adjacent
// swap of letters — Levenshtein counts that as distance 2 ("ceasar"
// vs "caesar" → e/a swapped at idx 1/2), but typographically it's
// the canonical single-character typo. Catching it separately lets
// us filter such typo tokens without having to widen the
// Levenshtein threshold to 2 (which would over-filter words like
// "cordon" that legitimately differ from a DICT word by two edits).
...
// gets a chance. Detect tokens that sit at edit distance 1 — OR a
// single adjacent transposition — from a hand-curated DICT word
// and skip them at index time. They're almost certainly typos and
// would only shadow the canonical entry.
function isTypoOfDictSingle(token) {
  for (const key of DICT_SINGLES) {
    ...
    if (hasAdjacentTransposition(token, key)) return true;
  }
  return false;
}
