...
function fuzzyAutoMatch(word) {
  if (!word) return null;
  if (word.length < 4) return null;
  const limit = word.length <= 5 ? 1 : 2;
  let bestKey = null;
  let bestDist = Infinity;
  for (const key of allAutoWords()) {
  ...
  return bestKey ? getAutoWordCode(bestKey) : null;
}
...
function extractKeywords(text) {
  ...
  for (const phrase of getAutoPhrases()) {
    const needle = ' ' + phrase + ' ';
    if (remaining.includes(needle)) {
      const code = getAutoPhraseCode(phrase);
  ...
  for (const word of remaining.split(/\s+/).filter(Boolean)) {
    ...
    const exactAuto = getAutoWordCode(word);
    if (exactAuto) { found.add(exactAuto); continue; }
    const fuzzyAuto = fuzzyAutoMatch(word);
...
function searchRecipe(query, recipes) {
  ...
  for (const [id, recipe] of Object.entries(recipes)) {
    ...
    // Tiebreak 2: prefer slug where the keyword appears earliest
