...
const { ARABIC_ALIASES } = require('./arabic-aliases');
const { normalizeText } = require('./normalize');
const { fuzzyMatchKeyword } = require('./fuzzy');

// arabic-aliases.js extends DICTIONARY with Boho-specific AR
// transliterations of dish names (Tzatziki, Bechamel, Mousse, …)
// the kitchen actually types. Combine the two lookup tables once at
// module load so the rest of search.js doesn't have to fan out.
const FULL_DICTIONARY = Object.assign({}, ARABIC_ALIASES, DICTIONARY);
const {
  rebuildAutoTerms,
  getAutoWordCode,
  getAutoPhrases,
...
// later lone "cheese" hit on the same text. Includes Arabic dish-name
// aliases ("كروك مدام", "تشيز كيك", …) via FULL_DICTIONARY.
const PHRASES = Object.keys(FULL_DICTIONARY)
  .filter((k) => k.includes(' '))
...
function addPhraseComponentSingles(phrase, foundSet, restaurant) {
  for (const w of phrase.split(/\s+/).filter(Boolean)) {
    ...
    if (FULL_DICTIONARY[w])                 { foundSet.add(FULL_DICTIONARY[w]); continue; }
...
function extractKeywords(text, restaurant) {
  ...
  for (const phrase of PHRASES) {
    ...
    if (remaining.includes(needle)) {
      found.add(FULL_DICTIONARY[phrase]);
  ...
  for (const word of remaining.split(/\s+/).filter(Boolean)) {
    if (FULL_DICTIONARY[word]) { found.add(FULL_DICTIONARY[word]); continue; }
