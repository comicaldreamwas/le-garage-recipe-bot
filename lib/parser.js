...
// Short stable hash for cache-vs-Notion diff detection.
function hashOf(text) {
  return crypto.createHash('md5').update(String(text || ''), 'utf8').digest('hex').slice(0, 12);
}
...
function classifySection(text) {
  ...
  // Step sub-headers like "Step 1 – Preparation of Ingredients" or
  // "1. Mixing" sit INSIDE the Preparation section and must classify
  // as prep, not ingredients — even though they may mention the word
  // 'ingredients'. Detect numbered prep step prefix first.
  const isStepHeading = /^(step\b|الخطوة\b|\d+[.)\-\s–]|[٠-٩]+[.)\-\s–])/i.test(cleaned);
  if (isStepHeading) {
...
function getHeaderText(block) {
  ...
  if (t === 'paragraph') {
    ...
    // Only promote when the paragraph is genuinely a LABEL: short (≤30
    // chars) AND has a label SHAPE — starts with an emoji, is ALL-CAPS,
    // or starts with an Arabic letter. Without the shape check, Boho
    // flat-bullet recipes get derailed: a body paragraph like
    // "Cooking cream 300ml" (contains the PREP keyword 'cooking') was
    // being treated as a section header and flipped state to prep,
    // dumping the rest of the bullets into the wrong bucket.
    const cls = classifySection(text);
    const isLabelShape = /^[\p{Extended_Pictographic}\p{Emoji_Presentation}]/u.test(text)
                      || /^[A-Z][A-Z0-9\s/&'()\-]{2,}$/.test(text)
                      || /^[؀-ۿ]/.test(text);
    if (text.length <= 30 && cls && isLabelShape) return text;
...
function composeRowLine(name, qty) {
  ...
  if (!q) return '• ' + n;
  if (!n) return '• ' + q;
  // Em-dash separates name from quantity for table rows — matches the
  // "Beef Fillet — 250 g" format the owner specified.
  return `• ${n} — ${q}`;
}
...
function splitBilingualLine(line) {
  ...
  // 4-segment bilingual: "EN_label / AR_label: EN_value / AR_value".
  // Common in storage-condition lines:
  //   "Storage Method / طريقة التخزين: In a sealed container / في علبة..."
  // Splitting on the first "/" alone leaves the EN_value buried inside
  // the AR_label segment (separated by ":"). Handle 3+ "/"-segments by
  // re-grouping all pieces by script.
  const segments = body.split(/[\/|]/).map((s) => s.trim()).filter(Boolean);
  if (segments.length >= 3) {
    const enBits = [];
    const arBits = [];
    for (const seg of segments) {
      if (hasArabic(seg) && hasLatin(seg) && seg.includes(':')) {
        const idx = seg.indexOf(':');
        const head = seg.slice(0, idx).trim();
        const tail = seg.slice(idx + 1).trim();
        if (head) {
          if (hasArabic(head) && !hasLatin(head)) arBits.push(head);
          else if (hasLatin(head) && !hasArabic(head)) enBits.push(head);
        }
        if (tail) {
          if (hasArabic(tail) && !hasLatin(tail)) arBits.push(tail);
          else if (hasLatin(tail) && !hasArabic(tail)) enBits.push(tail);
        }
      } else if (hasArabic(seg) && !hasLatin(seg)) {
        arBits.push(seg);
      } else if (hasLatin(seg) && !hasArabic(seg)) {
        enBits.push(seg);
      } else if (isArabicDominant(seg)) {
        arBits.push(seg);
      } else {
        enBits.push(seg);
      }
    }
    if (enBits.length && arBits.length) {
      return {
        en: prefix + enBits.join(': '),
        ar: prefix.replace(/\d+/, (m) => toArabicDigits(Number(m))) + arBits.join(': '),
      };
    }
  }
...
async function parseRecipe(page, topBlocks) {
  ...
  // Boho low-structure FLAG (no early-exit): pages that are pure
  // paragraphs (no headings, no bullets, no tables, no AR/EN
  // separator '——————') get marked so cache-builder can de-prioritise
  // them — but we still parse the paragraphs into ingredients_en so
  // unique-named recipes ("Whipped Cream", "Mashed Potatoes", …) are
  // kept rather than dropped. The label-shape tightening in
  // getHeaderText() prevents Pattern-1 sentences like "Cooking cream
  // 300ml" from spuriously flipping section to prep.
  let lowStructureFlag = false;
  if (page?.database_label === 'Boho') {
    const hasBullet    = blocks.some((b) => b.type === 'bulleted_list_item' || b.type === 'numbered_list_item');
    const hasTable     = blocks.some((b) => b.type === 'table' || b.type === 'table_row');
    const hasHeading   = blocks.some((b) => /^heading_/.test(b.type) || b.type === 'toggle' || b.type === 'callout');
    const hasSeparator = blocks.some((b) => {
      if (b.type !== 'paragraph') return false;
      const t = richTextToPlain(b.paragraph?.rich_text || []).trim();
      return /^[—\-]{4,}$/.test(t);
    });
    if (!hasBullet && !hasTable && !hasHeading && !hasSeparator) {
      lowStructureFlag = true;
    }
  }
  ...
  for (const block of blocks) {
    ...
    // Em-dash / hyphen separator paragraph ("————————"). Boho flat-bullet
    // recipes use it to mark the boundary between the EN block and the AR
    // block. When the EN side carried both ingredients and a short
    // preparation paragraph (e.g. Napoleon Cake), the AR bullets that
    // follow would otherwise inherit the 'prep' state and land in
    // prep_ar. Treat the separator as a hard reset to ingredients so
    // the AR bullets attach to ingredients_ar instead.
    if (type === 'paragraph') {
      const sepText = richTextToPlain(block.paragraph?.rich_text || []).trim();
      if (/^[—\-]{4,}$/.test(sepText)) {
        if (section === 'prep' || section === 'skip') section = 'ingredients';
        continue;
      }
    }
  ...
  // Exact-content mode (the bot serves what Notion actually has):
  // we intentionally do NOT run the "looks like an ingredient?" or
  // "is this a yield line?" content filters. Section detection and
  // dish-name dedupe still apply, so prep/ingredients stay separated
  // and the dish title doesn't double up — but anything Notion
  // actually wrote inside the ingredients section is kept verbatim.
  // Set EXACT_CONTENT=0 in .env to re-enable the legacy filters.
  if (process.env.EXACT_CONTENT === '0') {
    for (const key of ['ingredients_en', 'ingredients_ar', 'prep_en', 'prep_ar']) {
  ...
  const recipe = {
    ...
    restaurant: page.database_label
      ? page.database_label.toLowerCase().replace(/[^a-z]+/g, '_')
      : null,
    ...
    low_structure: lowStructureFlag,
  };
  // Field-level hashes used by the runtime and audit verifiers to
  // detect cache-vs-Notion drift without re-comparing full text.
  recipe.hashes = {
    name:           hashOf(recipe.name),
    ingredients_en: hashOf(recipe.ingredients_en),
    ingredients_ar: hashOf(recipe.ingredients_ar),
    prep_en:        hashOf(recipe.prep_en),
    prep_ar:        hashOf(recipe.prep_ar),
  };
  recipe.completeness = computeCompleteness(recipe);
  return recipe;
}

// Exposed so other modules (verifier, /verify, runtime check) hash
// identically to the parser.
function hashRecipeFields(recipe) {
  return {
    name:           hashOf(recipe?.name),
    ingredients_en: hashOf(recipe?.ingredients_en),
    ingredients_ar: hashOf(recipe?.ingredients_ar),
