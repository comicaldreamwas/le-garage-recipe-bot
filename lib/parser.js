...
// "cold storage") in recipe step titles.
const SKIP_TERMS = [
  'waiter', 'training', 'description', 'service notes', 'plating guide',
  'presentation', 'wine pairing', 'allergen', 'shelf life',
  'chef\'s note', 'quality check',
  'نادل', 'تدريب', 'ملاحظ',
];
...
function getHeaderText(block) {
  ...
  if (t === 'paragraph') {
    ...
    // "٣. التجهيز") are SUB-section markers inside whatever section is
    // already active — never section changes of their own. This stops
    // the parser from flipping into "prep" on "1. Cooking Process" or
    // skipping content because "3. Portioning..." substring-matches
    // a skip keyword.
    if (/^(\d+|[٠-٩]+)[.)]/.test(text)) return null;
    ...
    if (text.length <= 30 && classifySection(text)) return text;
  }
  return null;
}
