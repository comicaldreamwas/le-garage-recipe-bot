...
async function parseRecipe(page, topBlocks) {
  ...
  for (const block of topBlocks) {
    ...
    if (t.startsWith('heading_') || t === 'paragraph') {
      ...
      // Dish name: only take a heading when it clearly refers to the same
      // dish as the URL slug. Otherwise fall through to the slug name set
      // at the end. This avoids picking up section subheadings like
      // "Cooking for Service", "خطوات التنفيذ", or "🔥 1. Grilling" as
      // the recipe name.
      if (!out.name && t.startsWith('heading_') && !classifyHeader(text)) {
        ...
        const matchesSlug = slugTerms.some((term) => upperText.includes(term));
        if (matchesSlug) out.name = text;
      }
