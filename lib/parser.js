...
async function renderBlock(block, counters) {
  ...
  if (type === 'table' && block.has_children) {
    ...
    // Tables are special: cells often contain "EN / AR" pairs. We render
    // each row twice — once collecting the EN side, once the AR side —
    // and emit two lines tagged so the caller routes them correctly.
    const out = [];
    for (const r of rows) {
...
function splitBilingualCell(cell) {
  ...
  if (c.includes('/') && hasArabic(c) && hasLatin(c)) {
    ...
    // Heuristic: the EN piece is whichever side has Latin letters, AR is the rest.
  ...
  // Pure digits / units — duplicate to both sides
  return { en: c, ar: c };
}
