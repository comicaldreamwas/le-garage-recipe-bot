...
async function parseRecipe(page, topBlocks) {
  ...
  for (const block of topBlocks) {
    ...
    if (t.startsWith('heading_') || t === 'paragraph') {
      ...
      // a media label, or a generic guide title. Falls back to URL slug
      // (set below) when no good heading is found.
      if (!out.name && t.startsWith('heading_') && !classifyHeader(text)) {
        ...
        const isGeneric =
          lower.startsWith('cooking for') || lower.startsWith('recipe card') ||
          lower === 'recipe' || lower === 'menu item' ||
          lower.startsWith('service') || lower.startsWith('item') ||
          // Arabic equivalents of the same generic-guide titles
          text.startsWith('خطوات') || text.startsWith('التنفيذ') ||
          text.includes('أثناء الخدمة') || text.includes('إعداد للخدمة') ||
          text.startsWith('كارت وصفة') || text === 'وصفة';
        if (!isMediaLabel && !isGeneric) out.name = text;
      }
