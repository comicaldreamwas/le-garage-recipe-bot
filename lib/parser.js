...
async function parseRecipe(page, topBlocks) {
  ...
  for (const block of topBlocks) {
    ...
    if (t.startsWith('heading_') || t === 'paragraph') {
      ...
      // Dish name: only accept a heading when it clearly identifies the
      // same dish as the URL slug. Single-word matches are too loose
      // ("Step 3: Add Beef" matches BEEF-KEBAB on just BEEF), so we
      // require at least min(slugTerms, 2) overlapping terms. Recipes
      // whose slug is a single word still match on 1 term.
      if (!out.name && t.startsWith('heading_') && !classifyHeader(text)) {
        ...
        if (slugTerms.length > 0) {
          ...
          const hits = slugTerms.filter((term) => upperText.includes(term)).length;
          const required = Math.min(slugTerms.length, 2);
          if (hits >= required) out.name = text;
        }
      }
