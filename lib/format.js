...
const { bodyArabicName } = require('./arabicSearch');
...
function formatRecipe(recipe, options = {}) {
  ...
  // Drinks are intentionally single-language on some menus (the Boho bar
  // is English-only), so we don't flag a "missing" Arabic side that was
  // never expected — that warning is for the bilingual food recipes.
  const criticalGaps = recipe.type === 'drink' ? [] : missingCritical(recipe);
...
function buildFull(recipe) {
  ...
  // Authentic Arabic name (drinks carry it in-body) under the title so the
  // kitchen sees the name in both languages.
  const arName = bodyArabicName(recipe);
  if (arName) lines.push(`<b>${esc(arName)}</b>`);
  lines.push('');
